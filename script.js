...
(() => {
  ...
  const IA = {
    ...
    _cacheDOMElements() {
      ...
      this.elements.panelNav = document.querySelector(".panel-nav"); this.elements.tabButtons = document.querySelectorAll(".tab-button"); this.elements.tabPanels = document.querySelectorAll(".tab-panel"); this.elements.simulationControls = document.querySelector(".simulation-controls");
    },
    ...
    _bindEvents() {
        ...
        this._safeAddEventListener('forceActionButtons', "click", (e) => { if (e.target.matches('button')) { this._handleForcedAction(e.target.dataset.action); } });
    ...
    _safeAddEventListener(elementName, eventType, handler) { const element = this.elements[elementName]; if (element) { element.addEventListener(eventType, handler); } },
    _formatNumber(num) { return Math.round(num * 10) / 10; },
    _syncSandboxControls() { this.sandboxState = JSON.parse(JSON.stringify(this.state)); const syncGroup = (group, stateSource) => { for (const key in stateSource) { const sliderKey = `sandbox${key.charAt(0).toUpperCase() + key.slice(1)}`; const valueKey = `${sliderKey}Value`; const sliderElem = this.elements[sliderKey]; const valueElem = this.elements[valueKey]; if (sliderElem && valueElem) { const formattedValue = this._formatNumber(stateSource[key]); sliderElem.value = formattedValue; valueElem.textContent = formattedValue; } } }; syncGroup('attributes', this.sandboxState.attributes); syncGroup('needs', this.sandboxState.needs); },
    ...
    _updateUI() { for (const attr in this.state.attributes) { if (this.elements[attr]) { this.elements[attr].value = this.state.attributes[attr]; this.elements[`${attr}Value`].textContent = this.state.attributes[attr]; } } for (const need in this.state.needs) { if (this.elements[need]) { const formattedValue = this._formatNumber(this.state.needs[need]); this.elements[need].value = formattedValue; this.elements[`${need}Value`].textContent = formattedValue; } } if (this.elements.currentFeeling) { this.elements.currentFeeling.value = this.state.feelings.current; this.elements.lockFeelingToggle.checked = this.state.feelings.manualOverride; } ["cozinhar", "plantar", "dançar", "consertar"].forEach((key) => { if (this.elements[`knowledge${key.charAt(0).toUpperCase() + key.slice(1)}`]) { this.elements[`knowledge${key.charAt(0).toUpperCase() + key.slice(1)}`].checked = this.state.knowledge[key]; } }); this._updateRecipeBookUI(); this._updateFridgeUI(); this._updateItemsUI(); },
    ...
    _updateRecipeBookUI() { if(this.elements.recipeListContainer) { this.elements.recipeListContainer.classList.toggle("locked", !this.state.knowledge.cozinhar); this.elements.recipeList.innerHTML = Object.keys(CULINARY_BOOK).map((recipeName) => { const isKnown = this.state.knowledge.culinary.includes(recipeName); const ingredients = CULINARY_BOOK[recipeName].ingredients.join(", "); const checkedAttribute = isKnown ? 'checked' : ''; return ` <li class="list-group-item ${isKnown ? "known-recipe" : "unknown-recipe"}"> <div class="recipe-info"> <strong>${recipeName}</strong> <small>(${ingredients})</small> </div> <div class="knowledge-toggle form-check form-switch"><input class="form-check-input" type="checkbox" role="switch" data-recipe="${recipeName}" ${checkedAttribute}></div> </li> `; }).join(""); } },
